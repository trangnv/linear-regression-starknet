...
async def contract_factory():
    ...
    return starknet, contract, account1, account2, account3
...
@pytest.mark.asyncio
async def test_evaluation(contract_factory):
    """Test reveal_test_data method."""
    starknet, contract, account1, account2, account3 = await contract_factory
    ...
    await signer3.send_transaction(
        account3, contract.contract_address, "evaluation", []
    )
    # check max_error
    max_error_2 = await contract.view_max_error(account2.contract_address).call()
    # max_error calculation in python
    max_error_2_p = 0
    for i in range(len(X)):
        yhat = cal_yhat(X[i], model)
        error = abs(Y[i] - yhat)
        if max_error_2_p < error:
            max_error_2_p = error

    assert max_error_2.result.max_error == max_error_2_p
